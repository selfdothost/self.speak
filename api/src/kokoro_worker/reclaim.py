...
worker back, and it lazy-loads on the next /generate.
...
path, so killing it drops the whole TTS API rather than one engine.

**Kokoro is NOT the optional engine, and that changes the cost.** Stepping
aside costs chatterbox a slow voice clone; after the P3 cutover it costs
ORDINARY TTS a cold start — CUDA init plus a model load on the next request,
on the path that serves assistant devices. Still the right trade when a
priority-10 consumer needs the card, since the alternative is holding VRAM
against the inference brain forever. But it is a real availability cost, not
the shrug it is for chatterbox, so chatterbox should be asked to yield FIRST
(#5, P4). That ordering is NOT implemented here — P2 only makes this worker
capable of yielding at all.
"""
...
log = logging.getLogger("kokoro_worker.reclaim")

# Kill-switch. Set to 0 to make a forced release report its shortfall and keep
# the process alive.
RELEASE_MAY_EXIT = os.environ.get("KOKORO_RELEASE_MAY_EXIT", "1") not in (
    "0",
    "false",
    "False",
)
...
_EXIT_GRACE_SECONDS = float(os.environ.get("KOKORO_EXIT_GRACE_SECONDS", "1.5"))
...
def schedule_exit(reason: str) -> bool:
    ...
    log.warning(
        "kokoro-reclaim: stepping aside — %s. Exiting in %.1fs to return the "
    ...
    async def _die():
        ...
        log.warning("kokoro-reclaim: exiting now")
        os._exit(0)
